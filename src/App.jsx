...
/**
 * Vivid Decks v2.3
 * - Seed-Import: Decks werden nun additiv hinzugefügt (kein Überschreiben mehr)
 * - Persistenz: Bestehende Sammlung bleibt beim URL-Import erhalten
 * - UI: Icon-fokussiertes, minimalistisches Design
 * - Features: Erfolgsquote, Session-Protokoll, Tastatur-Support
 */
...
const App = () => {
  const [decks, setDecks] = useState(() => {
    ...
    const saved = localStorage.getItem('vivid_decks_v23');
    let currentDecks = saved ? JSON.parse(saved) : DEFAULT_DECKS;

    if (seed) {
      try {
        const decoded = JSON.parse(atob(decodeURIComponent(seed)));
        const newDeck = Array.isArray(decoded) ? decoded[0] : decoded;
        
        // Prüfen, ob das Deck bereits existiert (Anhand der ID oder des Namens)
        const exists = currentDecks.some(d => d.id === newDeck.id);
        if (!exists) {
          // Additiv hinzufügen: Neues Deck kommt an den Anfang
          currentDecks = [newDeck, ...currentDecks];
          // URL bereinigen ohne Neuladen (optional, verhindert doppeltes Hinzufügen beim Refresh)
          window.history.replaceState({}, document.title, window.location.pathname);
        }
      } catch (e) {
        console.error("Import-Fehler: Ungültiger Seed");
      }
    }
    return currentDecks;
  });
  ...
  useEffect(() => {
    localStorage.setItem('vivid_decks_v23', JSON.stringify(decks));
  }, [decks]);
  ...
  const checkAnswer = () => {
    ...
    const historyEntry = {
      ...
      input: userAnswer.trim() || "(Übersprungen)",
      isCorrect: correct
    };
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 font-sans pb-24 md:pb-0 ${darkMode ? 'bg-[#08080a] text-zinc-100' : 'bg-[#f4f7f9] text-zinc-900'}`}>
      ...
      <div className="fixed inset-0 pointer-events-none opacity-20 overflow-hidden z-0">
        ...
        <div className={`absolute bottom-0 right-0 w-[500px] h-[500px] blur-[150px] opacity-10 rounded-full ${darkMode ? 'bg-purple-600/30' : 'bg-purple-300/20'}`} />
      </div>

      {/* Navigation */}
      <nav className={`fixed z-[100] transition-all bottom-0 left-0 right-0 h-20 md:h-screen md:w-20 lg:w-64 md:top-0 md:border-r border-t md:border-t-0 flex md:flex-col items-center p-2 md:p-6 gap-6 ${darkMode ? 'bg-black/80 border-white/5' : 'bg-white/90 border-black/5 shadow-2xl'} backdrop-blur-xl`}>
        ...
        <div className="flex md:flex-col flex-1 w-full gap-2">
          {[
            ...
            <button 
              ...
              className={`flex-1 md:flex-none flex flex-col lg:flex-row items-center gap-3 p-3 lg:p-4 rounded-2xl transition-all ${
                activeView === item.id ? 'bg-indigo-600 text-white shadow-xl' : 'opacity-40 hover:opacity-100 hover:bg-white/5'
              }`}
            >
              <item.icon size={20} />
        ...
        <div className="max-w-6xl mx-auto p-6 md:p-12">
            ...
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <header className="flex justify-between items-end">
                ...
                <button onClick={() => {
                  ...
                  if(name) setDecks([{ id: Date.now(), name, description: 'Eigene Sammlung', cards: [] }, ...decks]);
                    ...
                    <div key={deck.id} className={`group p-8 rounded-[2.5rem] border transition-all hover:-translate-y-2 flex flex-col relative ${darkMode ? 'bg-zinc-900/40 border-white/5 hover:border-white/20' : 'bg-white border-black/5 shadow-xl'}`}>
                      <div className="flex justify-between items-start mb-10">
                        ...
                        <div className="flex gap-2 items-center">
                          <div className={`px-3 py-1.5 rounded-xl font-black text-xs flex items-center gap-2 ${
                            rate === 100 ? 'bg-green-500/10 text-green-500' : 'bg-indigo-500/10 text-indigo-500'
                          }`}>
                            <Percent size={14} /> {rate}%
                          </div>
                          <button onClick={() => copyShareLink(deck)} className="p-2 opacity-0 group-hover:opacity-100 transition-all text-zinc-400 hover:text-indigo-500" title="Teilen">
                            <Link2 size={18}/>
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="mb-8">
                        <h3 className="text-2xl font-black mb-1 italic tracking-tight leading-tight">{deck.name}</h3>
                        <p className="text-[10px] font-black uppercase opacity-20 tracking-widest">{deck.cards.length} Einheiten</p>
                      </div>

                      <div className="w-full h-1.5 bg-black/10 dark:bg-white/5 rounded-full overflow-hidden mb-10">
                        <div className={`h-full transition-all duration-1000 ${rate === 100 ? 'bg-green-500' : 'bg-indigo-500'}`} style={{ width: `${rate}%` }} />
                      </div>
                      ...
                      <div className="mt-auto flex gap-2">
                        ...
                        <button onClick={() => startSession(deck)} className="flex-1 py-4 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase flex items-center justify-center gap-2 hover:brightness-110 transition-all shadow-lg">
          ...
          {activeView === 'editor' && activeDeck && (
            <div className="animate-in fade-in duration-500 space-y-8">
              <button onClick={() => setActiveView('dashboard')} className="flex items-center gap-2 opacity-30 hover:opacity-100 font-black text-[10px] uppercase tracking-widest"><ChevronRight className="rotate-180" /> Dashboard</button>
              <h2 className="text-4xl font-black italic tracking-tighter">{activeDeck.name}</h2>

              <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1">
                   <div className={`p-8 rounded-[2rem] border sticky top-8 ${darkMode ? 'bg-zinc-900 border-white/5 shadow-2xl' : 'bg-white border-black/5 shadow-xl'}`}>
                      <h2 className="text-xl font-black mb-6 italic">Neue Karte</h2>
                      <div className="space-y-4">
                        ...
                        <button onClick={() => {
                            ...
                            if(f.value && b.value) { 
                              setDecks(decks.map(d => d.id === selectedDeckId ? {...d, cards: [{ id: Date.now(), front: f.value, back: b.value, box: 1 }, ...d.cards]} : d));
                              f.value = ''; b.value = ''; f.focus();
                            }
                          }} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl">Speichern</button>
                      </div>
                   </div>
                </div>
          ...
          {activeView === 'learn' && session && (
            <div className="max-w-3xl mx-auto py-4 space-y-8 animate-in fade-in zoom-in-95 duration-500">
              {!session.isFinished ? (
                <>
                  ...
                  <div className={`relative p-12 md:p-24 rounded-[4rem] border text-center transition-all duration-500 ${
                    isAnswerChecked 
                      ? isCorrect ? 'bg-green-500/10 border-green-500/30 shadow-2xl shadow-green-500/5' : 'bg-rose-500/10 border-rose-500/30 shadow-2xl shadow-rose-500/5'
                      : (darkMode ? 'bg-zinc-900 border-white/5' : 'bg-white border-black/5 shadow-2xl shadow-black/10')
                  }`}>
                    
                    {isAnswerChecked && (
                      <div className={`absolute top-10 left-1/2 -translate-x-1/2 px-6 py-2 rounded-full font-black text-[10px] uppercase tracking-[0.4em] flex items-center gap-3 animate-in slide-in-from-top-4 ${
                        isCorrect ? 'bg-green-500 text-white shadow-lg' : 'bg-rose-500 text-white shadow-lg'
                      }`}>
                    ...
                    <div className="max-w-sm mx-auto space-y-4">
                      <input 
                        ...
                        className={`w-full py-6 px-10 rounded-3xl text-center text-xl font-black bg-black/20 border-2 outline-none transition-all ${
                          isAnswerChecked ? (isCorrect ? 'border-green-500 text-green-500' : 'border-rose-500 text-rose-500') : 'border-white/10 focus:border-indigo-500'
                        }`}
                      />
                      ...
                      <div className="pt-6">
                        {!isAnswerChecked ? (
                          ...
                          <button onClick={nextCard} className={`w-full py-6 rounded-3xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 transition-transform active:scale-95 shadow-lg ${isCorrect ? 'bg-green-600 text-white' : 'bg-zinc-800 text-white'}`}>
                ...
                <div className="space-y-8 animate-in zoom-in-95 duration-500">
                  <div className={`p-16 text-center space-y-8 rounded-[4rem] border ${darkMode ? 'bg-zinc-900 border-white/5 shadow-2xl' : 'bg-white border-black/5 shadow-xl'}`}>
                    ...
                    <h2 className="text-5xl font-black italic tracking-tighter uppercase leading-none">Abgeschlossen</h2>
                    <div className="flex gap-4 max-w-xs mx-auto">
                      <div className="flex-1 p-6 rounded-3xl bg-green-500/10 border border-green-500/20">
                        <p className="text-3xl font-black text-green-500 leading-none">{session.results.correct}</p>
                      </div>
                      <div className="flex-1 p-6 rounded-3xl bg-rose-500/10 border border-rose-500/20">
                        <p className="text-3xl font-black text-rose-500 leading-none">{session.results.wrong}</p>
                      </div>
                    </div>
          ...
          {activeView === 'stats' && (
            <div className="animate-in fade-in slide-in-from-left-4 duration-500 space-y-12">
               <h2 className="text-6xl font-black italic tracking-tighter leading-none">Status</h2>
          ...
          {activeView === 'settings' && (
            <div className="max-w-2xl mx-auto space-y-12 animate-in fade-in duration-500">
               <h2 className="text-6xl font-black italic tracking-tighter leading-none">Optionen</h2>
               <div className={`p-10 rounded-[3rem] border space-y-6 ${darkMode ? 'bg-zinc-900/40 border-white/5' : 'bg-white border-black/5 shadow-xl'}`}>
                 ...
                 <button onClick={() => { if(confirm('Soll die App auf Werkseinstellungen zurückgesetzt werden? Alle eigenen Decks gehen verloren.')) { setDecks(DEFAULT_DECKS); notify('Reset'); }}} className="w-full p-6 rounded-2xl border border-rose-500/20 text-rose-500 font-black text-[10px] uppercase tracking-widest hover:bg-rose-500/5 transition-all">Alles Zurücksetzen</button>
               </div>
            </div>
          )}
